...
- **Fish identity**: each agent gets a persistent name (e.g. `redfish-a3b2`) with tracked cookies, domain state, CF clearance. Survives restarts via JSON snapshot.
...
python examples/stress_test.py       # reliability suite (needs make run or run-cf)
...
- `cmd/controller/main.go` — controller binary, flag parsing, graceful shutdown
- `cmd/agent/main.go` — agent binary, backend selection
- `internal/controller/server.go` — HTTP handlers, cookie handoff, renewal
- `internal/controller/pool.go` — agent pool, leases, warm matching, reconnection
- `internal/controller/identity.go` — fish names, domain state, CF tracking
- `internal/controller/health.go` — background health checks, dead agent removal
...
- `internal/agent/cdp.go` — CDP backend (Lightpanda, generic), persistent tab, CF challenge wait, actions
